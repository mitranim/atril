...
function compileTextNode(node) {
    ...
    if (node.parentNode && node.parentNode instanceof HTMLTextAreaElement)
        return;
...
function unpackTemplatesFromMolds(element) {
    ...
    for (var i = attributes.length - 1; i >= 0; --i) {
        ...
        if (utils.looksLikeCustomAttribute(attr.name)) {
            ...
            if (decorators_1.registeredMolds[partialName]) {
                if (!atCapacity) {
                    atCapacity = true;
                    continue;
                }
...
function compileMoldsOnTemplate(template) {
    ...
    for (var i = 0, ii = template.attributes.length; i < ii; ++i) {
        ...
        if (utils.looksLikeCustomAttribute(attr.name)) {
            ...
            var VM = decorators_1.registeredAttributes[partialName];
            utils.assert(!!VM, "no registered attribute found for '" + attr.name + "' on template:", template);
            utils.assert(decorators_1.registeredMolds[partialName], "unexpected non-mold '" + attr.name + "' on template:", template);
            // No more than one.
...
function compileAttributeBindingsOnRealElement(virtual, real) {
    ...
    var attributes = [].slice.call(real.attributes, 0);
    if (utils.browserReversesAttributes)
        attributes = attributes.reverse();
    for (var _i = 0; _i < attributes.length; _i++) {
        ...
        if (utils.looksLikeCustomAttribute(attr.name)) {
            ...
            utils.assert(!decorators_1.registeredMolds[partialName], "unexpected mold '" + attr.name + "' on element:", real);
            // Register binding.
            if (!meta.dynamic)
                meta.markDynamic();
