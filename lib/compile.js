...
function compileNode(node) {
    ...
    tree_1.getOrAddTrace(node);
}
exports.compileNode = compileNode;
function compileTextNode(node) {
    var trace = tree_1.getOrAddTrace(node);
...
function compileElement(element) {
    var trace = tree_1.getOrAddTrace(element);
...
function compileMoldsOnTemplate(template) {
    var trace = tree_1.getOrAddTrace(template);
    if (trace.compiled)
        return;
...
function compileAttributeBindingsOnRealElement(virtual, real) {
    var trace = tree_1.getOrAddTrace(virtual);
