...
var If = (function () {
    function If() {
        ...
        while (container.hasChildNodes()) {
            ...
            tree_1.getOrAddState(child).isDomImmutable = true;
    ...
    For.prototype.step = function (value, index) {
        ...
        if (this.stash.length >= this.originals.length) {
            ...
            nodes = this.originals.map(function (node) {
                ...
                tree_1.getOrAddState(clone).isDomImmutable = true;
                return clone;
            });
        }
        var state = tree_1.getOrAddState(nodes[0]);
        if (!state.scope)
        ...
        while (nodes.length) {
            ...
            tree_1.getOrAddState(node).scope = state.scope;
...
var Ref = (function () {
    function Ref() {
        console.assert(!this.hint || this.hint === 'vm', "expected 'ref.' or 'ref.vm', got: 'ref." + this.hint + "'");
...
var Declare = (function () {
    function Declare() {
        console.assert(/^[$_A-Za-z]+[$_A-Za-z0-9]*$/.test(this.hint), "'declare.*' expects the hint to be a valid JavaScript identifier, got: " + this.hint);
        var attributeName = 'declare.' + this.hint;
        console.assert(!this.element.getAttribute(attributeName), "'declare.*' doesn't accept an expression");
        // Make sure the scope is available.
        if (!this.scope) {
            var state = tree_1.getOrAddState(this.element);
            state.scope = Object.create(null);
            this.scope = state.scope;
        }
        // Bring the identifier into scope.
        if (!(this.hint in this.scope))
            this.scope[this.hint] = undefined;
